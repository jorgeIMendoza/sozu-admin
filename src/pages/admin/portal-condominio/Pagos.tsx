...
import { pagos, formatMXN } from "@/data/portalCondominio/mockData";

export default function Pagos() {
  const [estatus, setEstatus] = useState("todos");
  const filtered = useMemo(() => pagos.filter((p) => estatus === "todos" || p.estatus_conciliacion === estatus), [estatus]);
  const totalConciliado = pagos.filter((p) => p.estatus_conciliacion === "conciliado").reduce((s, p) => s + p.monto, 0);
  const totalExcepcion = pagos.filter((p) => p.estatus_conciliacion === "excepcion" || p.estatus_conciliacion === "pendiente").reduce((s, p) => s + p.monto, 0);
