...
const ClienteMantenimientoPago = () => {
  ...
  if (!prop || !prop.mantenimientoClabeStp) {
    return (
      <div className="max-w-lg mx-auto px-5 pt-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          <ArrowLeft className="w-4 h-4" /> Regresar
        </button>
        <p className="text-muted-foreground">No se encontró información de pago para mantenimiento.</p>
      </div>
    );
  }

  const clabe = prop.mantenimientoClabeStp;
  const montoSugerido = prop.cuotaMensualMantenimiento;
  const concepto = `${prop.proyecto}-${prop.unidad}-MANTO`;
...
export default ClienteMantenimientoPago;
