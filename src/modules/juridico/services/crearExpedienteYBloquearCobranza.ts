...
export interface CrearExpedienteYBloquearCobranzaInput {
  /** ID de la cuenta de cobranza principal (BIGINT — string o number). */
  idCuentaCobranza: string | number;
...
export interface CrearExpedienteYBloquearCobranzaResult {
  ...
  idTipoAsunto: string;
  /** ID de la propiedad que quedó bloqueada (id_estatus_disponibilidad=11). */
  idPropiedadBloqueada: string;
}
...
function assertValidEnvelope(data: unknown): asserts data is {
  ...
  if (
    ...
    typeof d?.idPropiedadBloqueada !== 'string'
  ) {
...
export async function crearExpedienteYBloquearCobranza(
  input: CrearExpedienteYBloquearCobranzaInput,
): Promise<CrearExpedienteYBloquearCobranzaResult> {
  const { data, error } = await (supabase as any).rpc('crear_expediente_y_bloquear_cobranza', {
    p_id_cuenta_cobranza: input.idCuentaCobranza,
  ...
  return {
    ...
    idPropiedadBloqueada: data.data.idPropiedadBloqueada,
  };
}
