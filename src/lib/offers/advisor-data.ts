...
export function getAdvisorForProperty(propertyId: string, phaseOverride?: PhaseOrFlow): Advisor {
  ...
  const inv = mockPortfolio.find((p) => p.property.id === propertyId);
  if (!inv) return useAdvisorStore.getState().roster[0];
  const activeStage = inv.stages.find((s) => s.status === "active");
  return getAdvisorByPhase((activeStage?.id ?? "preventa") as StageId);
}
...
export function useAdvisorForProperty(propertyId: string, phaseOverride?: PhaseOrFlow): Advisor {
  return useAdvisorStore((s) => {
    if (phaseOverride) {
      return resolveAdvisor(s.roster, phaseOverride);
    }
    const inv = mockPortfolio.find((p) => p.property.id === propertyId);
    if (!inv) return s.roster[0];
    const activeStage = inv.stages.find((st) => st.status === "active");
    const phase = (activeStage?.id ?? "preventa") as StageId;
    return resolveAdvisor(s.roster, phase);
  });
}
