...
const IncidentDetailSheet = ({ incidentId, open, onClose }: Props) => {
  const incident = usePostDeliveryStore((s) =>
    s.incidents.find((i) => i.id === incidentId),
  );
  const closeIncident = usePostDeliveryStore((s) => s.closeIncident);
        ...
        <div className="px-5 py-5 mt-4">
          {incident.status === "resuelto" && (
            <button
              ...
              className="w-full h-12 rounded-xl bg-success text-success-foreground font-semibold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
            >
