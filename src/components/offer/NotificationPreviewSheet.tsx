...
const NotificationPreviewSheet = ({
  ...
  return (
    ...
    >
      <div
        role="dialog" aria-modal="true" aria-labelledby="notification-preview-title"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-border">
          <div className="min-w-0">
            ...
            <h2 id="notification-preview-title" className="text-base font-semibold text-foreground mt-0.5">{template.label}</h2>
          </div>
          <button
            onClick={onClose}
            aria-label="Cerrar"
            className="w-11 h-11 rounded-lg hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {hasEmail && hasWhatsApp && (
          <div className="px-5 pt-3">
            <div role="tablist" className="flex gap-1 p-1 rounded-lg bg-muted">
              <button
                role="tab" aria-selected={activeTab === "email"}
                onClick={() => setActiveTab("email")}
                className={`flex-1 h-11 rounded-md text-xs font-semibold flex items-center justify-center gap-2 transition-colors ${
                  activeTab === "email"
              ...
              <button
                role="tab" aria-selected={activeTab === "whatsapp"}
