...
import { Maximize2, X, ZoomIn, ZoomOut, Square, Bed, Bath, Building2 } from "lucide-react";
...
const OfferFloorPlanLarge = ({ imageUrl, unitArea, bedrooms, bathrooms, view, floor }: Props) => {
  ...
  const handleClose = () => { setZoom(1); setOpen(false); };
  ...
  return (
    <>
      <div className="rounded-2xl border border-border bg-card p-5 md:p-6 space-y-4">
        <div className="flex items-end justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[11px] font-semibold tracking-[0.18em] uppercase text-muted-foreground">Plano arquitectónico</h2>
          </div>
      ...
      {open && (
        <div onClick={handleClose} className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4">
          <button onClick={handleClose} aria-label="Cerrar" className="absolute top-4 right-4 w-11 h-11 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/20 z-10">
            <X className="w-5 h-5" />
          </button>
          <div onClick={(e) => e.stopPropagation()} className="absolute top-4 left-4 flex items-center gap-2 z-10">
            <button onClick={() => setZoom((z) => Math.max(z - 0.5, 1))} disabled={zoom <= 1} aria-label="Reducir" className="w-11 h-11 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/20 disabled:opacity-40 disabled:cursor-not-allowed">
              <ZoomOut className="w-5 h-5" />
            </button>
            <span className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-md text-white text-xs font-semibold tabular-nums min-w-[60px] text-center">{Math.round(zoom * 100)}%</span>
            <button onClick={() => setZoom((z) => Math.min(z + 0.5, 4))} disabled={zoom >= 4} aria-label="Aumentar" className="w-11 h-11 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/20 disabled:opacity-40 disabled:cursor-not-allowed">
              <ZoomIn className="w-5 h-5" />
            </button>
          </div>
          <div onClick={(e) => e.stopPropagation()} className="w-full h-full max-w-6xl max-h-[85vh] overflow-auto flex items-center justify-center">
            <img src={imageUrl} alt="Plano arquitectónico ampliado" style={{ transform: `scale(${zoom})`, transformOrigin: "center center" }} className="max-w-full max-h-full object-contain transition-transform duration-200" />
          </div>
