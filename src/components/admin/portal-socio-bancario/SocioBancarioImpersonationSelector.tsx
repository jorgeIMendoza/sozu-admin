...
import {
  ...
  SOCIO_BANCARIO_IMPERSONATED_ROL_ID,
  type SocioBancarioImpersonatedUser,
 ...
 * Visible solo para usuarios con roles.puede_impersonar. Lista usuarios con
 * rol Administrador de Proyecto (rol_id = 2).
 */
export function SocioBancarioImpersonationSelector() {
  const { profile } = useAuth();
  const { impersonatedUser, setImpersonatedUser, clearImpersonation, isImpersonating } =
    useSocioBancarioImpersonation();
  const [open, setOpen] = useState(false);
  const [users, setUsers] = useState<SocioBancarioImpersonatedUser[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        ...
        .eq("rol_id", SOCIO_BANCARIO_IMPERSONATED_ROL_ID)
        .eq("activo", true)
        .order("nombre");
      if (data) {
        setUsers(
          (data as Array<any>).map((u) => ({
            id: String(u.id),
            nombre: u.nombre || u.email,
            email: u.email,
            rol_id: u.rol_id as number,
            ...
            <CommandList>
              ...
              <CommandGroup>
                ...
                >
                  ...
                  <span className="font-medium">Vista por defecto</span>
                </CommandItem>
                {users.map((u) => (
                  <CommandItem
                    key={u.id}
