 ...
 import { Upload, Download, CheckCircle, XCircle, AlertCircle, Loader2, Copy, Info } from 'lucide-react';
 import { supabase } from '@/integrations/supabase/client';
 import { toast } from 'sonner';
 import { useActivityLogger } from '@/hooks/useActivityLogger';
 import { Alert, AlertDescription } from '@/components/ui/alert';
 
 interface BulkUploadMisAgentesDialogProps {
   open: boolean;
   onClose: () => void;
   onSuccess: () => void;
 ...
 export function BulkUploadMisAgentesDialog({ 
   open, 
   onClose, 
   onSuccess, 
   inmobiliariaId,
   inmobiliariaNombre 
 }: BulkUploadMisAgentesDialogProps) {
     ...
     for (let i = 1; i < lines.length; i++) {
       ...
       if (values.length >= 3) {
         agents.push({
           ...
           inmobiliaria: inmobiliariaNombre, // Auto-filled
           inmobiliariaId: inmobiliariaId, // Pre-filled ID
         });
       }
     }
   ...
   const handleUpload = async () => {
     ...
     try {
       ...
       setProgress(30);
       console.log(`Procesando ${agents.length} agentes para inmobiliaria ${inmobiliariaNombre}...`);
   ...
   const downloadTemplate = () => {
     const csvContent = 'Nombre,Telefono,Email\nNombre De Ejemplo,3333333333,correo@correo.com';
     const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
     const link = document.createElement('a');
     link.href = URL.createObjectURL(blob);
     link.download = 'template_mis_agentes.csv';
     link.click();
   };
     ...
     <Dialog open={open} onOpenChange={handleClose}>
       <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
         <DialogHeader>
           ...
           <DialogDescription>
             Sube un archivo CSV con los datos de los agentes a crear para tu inmobiliaria.
           </DialogDescription>
         </DialogHeader>
         ...
         <div className="space-y-4">
           ...
           <Alert>
             ...
             <AlertDescription>
               Los agentes se crearán para la inmobiliaria: <strong>{inmobiliariaNombre}</strong>
               <br />
               <span className="text-sm text-muted-foreground">
                 Los agentes heredarán automáticamente los accesos a proyectos de tu inmobiliaria.
               </span>
             </AlertDescription>
           </Alert>
           ...
           <div className="space-y-2">
             <Label htmlFor="csv-file-mis-agentes">Archivo CSV</Label>
             <Input
               id="csv-file-mis-agentes"
               type="file"
               accept=".csv"
           ...
           {results && (
             <div className="space-y-3">
               ...
               {!results.success && results.phase === 'execution_rollback' && (
                 <div className="p-3 bg-destructive/10 rounded border border-destructive/30 space-y-3">
                   ...
                   <p className="text-sm text-destructive/80">
                     {results.message || 'Ocurrió un error. Todos los cambios fueron revertidos.'}
                   </p>
                 </div>
               )}
