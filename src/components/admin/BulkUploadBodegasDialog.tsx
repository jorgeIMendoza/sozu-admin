...
export const BulkUploadBodegasDialog = ({ 
  open, 
  onClose, 
  onSuccess 
}: BulkUploadBodegasDialogProps) => {
  ...
  const resetFile = () => {
    setFile(null);
    const fileInput = document.getElementById('csv-file-input-bodegas') as HTMLInputElement;
    if (fileInput) {
      fileInput.value = '';
    }
  };
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Carga Masiva de Bodegas
          </DialogTitle>
          <DialogDescription>
            Sube un archivo CSV para cargar múltiples bodegas de forma masiva.
          </DialogDescription>
        </DialogHeader>
          ...
          {!isUploading ? (
            <>
              <div className="flex items-center justify-center w-full">
                <label
                  htmlFor="csv-file-input-bodegas"
