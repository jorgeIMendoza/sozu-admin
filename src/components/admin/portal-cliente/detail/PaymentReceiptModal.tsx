...
const PaymentReceiptModal = ({ receipt, open, onClose }: PaymentReceiptModalProps) => {
  ...
  const handleDownload = async () => {
    if (!receipt.pagoId || downloading) return;
    setDownloading(true);
    try {
      const res = await fetch(`${PROD_FUNCTIONS_BASE_URL}/generar-recibo-pago`, {
        ...
        body: JSON.stringify({ pagoId: receipt.pagoId }),
      });
      const data = await res.json();
      if (!res.ok || !data?.url_recibo) return;
      const blob = await fetch(data.url_recibo).then(r => r.blob());
      const objUrl = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = objUrl;
      a.download = `SOZU-Recibo-${receipt.folio}.pdf`;
      a.click();
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        className={[
          // hide Radix default close button
          "[&>button:last-child]:hidden",
          "p-0 gap-0 border-border flex flex-col overflow-hidden",
          // mobile: bottom sheet at 75%
      ...
      >
        ...
        <div className="overflow-y-auto flex-1">
          {/* Brand header */}
          <div className="bg-primary/5 border-b border-border px-6 pt-5 pb-4">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-card rounded-xl border border-border p-4 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-semibold text-foreground tabular-nums">{fmt(receipt.totalAssetValue)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">Total pagado acumulado</span>
                  <span className="text-sm font-semibold text-primary tabular-nums">{fmt(receipt.totalPaidAccumulated)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">Saldo pendiente</span>
                  <span className="text-sm font-semibold text-foreground tabular-nums">{fmt(receipt.pendingBalance)}</span>
                </div>
...
function DetailRow({
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 gap-3">
      ...
      <span className={`text-xs font-semibold text-foreground text-right ${mono ? "font-mono" : ""}`}>
        {value}
      </span>
    </div>
  );
