...
export const BulkUploadEstacionamientosDialog = ({ 
  open, 
  onClose, 
  onSuccess 
}: BulkUploadEstacionamientosDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast({
        title: "Carga exitosa",
        description: "El archivo de estacionamientos se ha procesado correctamente.",
      });
  ...
  const resetFile = () => {
    setFile(null);
    const fileInput = document.getElementById('csv-file-input-estacionamientos') as HTMLInputElement;
    if (fileInput) {
      fileInput.value = '';
    }
  };
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Carga Masiva de Estacionamientos
          </DialogTitle>
          <DialogDescription>
            Sube un archivo CSV para cargar múltiples estacionamientos de forma masiva.
          </DialogDescription>
        </DialogHeader>
          ...
          {!isUploading ? (
            <>
              <div className="flex items-center justify-center w-full">
                <label
                  htmlFor="csv-file-input-estacionamientos"
                ...
                >
                  ...
                  <input
                    id="csv-file-input-estacionamientos"
                    type="file"
                    className="hidden"
                    accept=".csv"
