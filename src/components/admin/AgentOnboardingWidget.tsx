...
import { Check, Trophy, Loader2, ChevronDown } from "lucide-react";
...
export function AgentOnboardingWidget({ personaId, variant = 'default' }: AgentOnboardingWidgetProps) {
  ...
  return (
    <>
      <Collapsible open={expanded} onOpenChange={setExpanded} className={cn(isInline ? "" : "bg-card border-b px-4 py-3")}>
        {/* Collapsed: just the percentage bar + toggle */}
        <CollapsibleTrigger className="flex items-center gap-3 w-full cursor-pointer group">
