...
export function NivelMorosidad({ items, acumulado }: { items: MorosidadItem[]; acumulado?: boolean }) {
  return (
    <section>
...
export function AgingYRiesgo({ aging, projectRows, onSelectProject, acumulado }: {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {aging && aging.length > 0 && (
          <div className="sozu-kpi-card">
            <h3 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground mb-4 flex items-center gap-1.5">Antigüedad de cartera{acumulado && <AcumuladoTag />}</h3>
        ...
        <div className="sozu-kpi-card">
          <h3 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground mb-4 flex items-center gap-1.5">Riesgo por proyecto{acumulado && <AcumuladoTag />}</h3>
          {top.length > 0 ? (
...
export function ClientesCriticos({ title, badgeSuffix, count, accounts, onSelect, onSeeAll, acumulado }: {
