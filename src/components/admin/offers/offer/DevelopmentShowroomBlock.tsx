  ...
  const fullAddressLine =
    showroom.city || showroom.state || showroom.zipCode
      ? `${showroom.city ?? ""}${showroom.state ? `, ${showroom.state}` : ""}${
          showroom.zipCode ? ` ${showroom.zipCode}` : ""
        }${showroom.country ? `, ${showroom.country}` : ""}`
      : "";
    ...
    <div className="rounded-2xl border-2 border-primary/20 bg-card overflow-hidden">
      ...
      <div className="p-5 md:p-6">
        ...
        <div className="mb-5 md:ml-12">
          ...
          {fullAddressLine && (
            <p className="text-xs text-muted-foreground mt-0.5">{fullAddressLine}</p>
          )}
        </div>
        ...
        {showroom.schedule && showroom.schedule.length > 0 && (
          <div className="md:ml-12 mb-5">
            ...
            {scheduleNote && (
              <p className="text-[10px] text-muted-foreground mt-2 italic">{scheduleNote}</p>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-2 md:ml-12">
          ...
          <a
            ...
            rel="noopener noreferrer"
            className={`${
              bookingLink ? "flex-1" : "w-full"
            } h-11 rounded-xl bg-card border border-border text-foreground text-sm font-semibold hover:border-foreground/30 transition-colors flex items-center justify-center gap-2`}
          >
