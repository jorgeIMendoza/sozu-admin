...
import type { DevelopmentInfo, InstagramPost } from "@/lib/offers/offer-data";
...
const SocialBadge = ({
  ...
  <a
    ...
    className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-full bg-card border border-border text-xs font-semibold transition-colors ${colorClass}`}
  >
...
const InstagramPostCard = ({ post }: { post: InstagramPost }) => (
  <a
    ...
    target="_blank"
    rel="noopener noreferrer"
    className="relative block aspect-square rounded-xl overflow-hidden border border-border bg-muted group"
  >
    <img
      src={post.imageUrl}
...
const DevelopmentPresenceSection = ({ development, developmentName, agent }: Props) => {
  ...
  const hasContent =
    website || socials || (instagramPosts && instagramPosts.length > 0) || showroom;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {website && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 p-4 rounded-xl border border-border bg-background hover:border-primary/40 hover:bg-muted/40 transition-colors group"
          >
            ...
            <div className="flex flex-wrap gap-2">
              {socials.instagram && (
                <SocialBadge
                  ...
                  colorClass="text-pink-600 dark:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-950/30"
                />
              )}
              {socials.facebook && (
                <SocialBadge
                  ...
                  icon={Facebook}
                  label="Facebook"
                  colorClass="text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30"
                />
              )}
              {socials.youtube && (
                <SocialBadge
                  ...
                  icon={Youtube}
                  label="YouTube"
                  colorClass="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30"
                />
              )}
            </div>
          </div>
