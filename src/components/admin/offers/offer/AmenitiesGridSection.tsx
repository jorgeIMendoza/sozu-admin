  ...
  return (
    <>
      <section className="rounded-2xl bg-card border border-border overflow-hidden">
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 auto-rows-[140px]">
...
const AmenityCard = ({ amenity, onClick }: AmenityCardProps) => {
  const sizeClass = SIZE_CLASSES[amenity.size];
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative overflow-hidden rounded-xl border border-border-subtle bg-muted text-left transition-all hover:scale-[1.02] hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${sizeClass}`}
    >
      {firstImage ? (
        <>
          <img
          ...
          <div className="absolute inset-0 p-4 flex flex-col justify-end">
            <div className="flex items-center gap-1.5 mb-1">
              <IconComponent className="w-3.5 h-3.5 text-background" />
              <h4 className="text-sm font-bold text-background leading-tight">{amenity.name}</h4>
            </div>
            <p className="text-[11px] text-background/85 leading-snug line-clamp-2">
