...
const DEFAULT_AGENT_ID = "AGT-RAMON";
const DEFAULT_ADMIN_ID = "AGT-LUZ";

const PropertyHumanContact = ({ investment, role }: Props) => {
  const { property } = investment;
  const contactId = role === "agent" ? DEFAULT_AGENT_ID : DEFAULT_ADMIN_ID;
  const contact = useAgentById(contactId);

  const sectionTitle =
    role === "agent" ? "Tu agente comercial" : "Tu administrador de patrimonio";
    ...
    return (
      <section className="rounded-2xl bg-card border border-border p-5 md:p-6">
        ...
        <p className="text-[13px] text-muted-foreground">
          Tu {role === "agent" ? "agente comercial" : "administrador"} se asignará próximamente.
        </p>
      </section>
    );
  }
  ...
  const roleDescription =
    role === "agent"
      ...
      : "Gestiona el mantenimiento, plusvalía y uso de tu propiedad.";
