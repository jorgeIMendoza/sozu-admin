## Reparar/registrar menú de Activos Comerciales bajo Inventarios — 2026-07-03
...
WITH menu_inventarios AS (
  SELECT id
  FROM public.menus
  WHERE lower(trim(nombre)) = lower('Inventarios')
  LIMIT 1
),
menu_actualizado AS (
  ...
  WHERE m.id IN (SELECT id FROM menu_inventarios)
  RETURNING m.id
),
menu_creado AS (
  INSERT INTO public.menus (nombre, orden, activo)
  SELECT 'Inventarios', 6, true
  WHERE NOT EXISTS (SELECT 1 FROM menu_inventarios)
  RETURNING id
),
menu_objetivo AS (
...
submenu_creado AS (
  ...
  SELECT (SELECT id FROM menu_objetivo), 'Activos Comerciales', '/admin/activos-comerciales', 8, true, false
...
> Este script es idempotente: si el menú/submenú ya existe, reactiva `menus.activo`, activa `menus_roles`, mueve el submenú a Inventarios y completa permisos disponibles + permisos Super Admin.
