...
-- Drop the broken new overload (with 12 params)
DROP FUNCTION IF EXISTS public.get_inventario_disponible_v2(
  int[], text[], text[], int[], text[], boolean, boolean, text, int, int, numeric, numeric
);

-- Drop the old function (with 10 params)
DROP FUNCTION IF EXISTS public.get_inventario_disponible_v2(
  int[], text[], text[], int[], text[], boolean, boolean, text, int, int
);

-- Recreate single function with price params
