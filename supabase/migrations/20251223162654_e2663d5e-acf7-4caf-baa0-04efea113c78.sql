...
SET query_sql = 'SELECT
  CASE WHEN o.id_producto IS NOT NULL THEN ''CCP-'' || LPAD(cc.id::text, 6, ''0'') ELSE ''CC-'' || LPAD(cc.id::text, 6, ''0'') END AS numero_cuenta,
  ...
  cc.fecha_compra,
  cc.clabe_stp,
  MAX(ap.fecha_pago) AS fecha_pago_maxima_vencida,
...
LEFT JOIN acuerdos_pago ap ON ap.id_cuenta_cobranza = cc.id AND ap.activo = true AND ap.fecha_pago <= CURRENT_DATE
LEFT JOIN (
...
WHERE cc.activo = true
  ...
  {{AND (CASE WHEN o.id_producto IS NOT NULL THEN ''Producto'' ELSE ''Propiedad'' END) = :tipo}}
