-- Fix report ID 5: use correct table name entidades_relacionadas instead of entidades_proyectos
UPDATE reportes
SET query_sql = 'SELECT
  ...
  COALESCE(prop.numero_propiedad, '''') AS numero_propiedad,
...
GROUP BY cc.id, o.id_producto, proy.nombre, em.nombre, mod.nombre, prop.numero_propiedad, prod.nombre, per.nombre_legal, dueno_persona.nombre_legal, cc.precio_final, cc.fecha_compra, cc.clabe_stp, pagado.total_pagado
ORDER BY proy.nombre, em.nombre, prop.numero_propiedad'
WHERE id = 5;