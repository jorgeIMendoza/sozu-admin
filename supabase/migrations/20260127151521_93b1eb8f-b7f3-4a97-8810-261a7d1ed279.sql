...
-- Drop existing function first to fix the column reference error
DROP FUNCTION IF EXISTS get_propiedades_paginadas(INTEGER, INTEGER, TEXT, INTEGER[], INTEGER[], INTEGER, INTEGER, INTEGER[], INTEGER[], NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, BOOLEAN, BOOLEAN, TEXT, INTEGER[], INTEGER[]);

-- Recreate with fixed compradores_info CTE (using fecha_creacion instead of non-existent id column)
CREATE FUNCTION get_propiedades_paginadas(
  p_page INTEGER,
  p_per_page INTEGER,
