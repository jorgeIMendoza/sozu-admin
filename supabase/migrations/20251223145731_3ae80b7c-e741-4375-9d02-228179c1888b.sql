-- Fix the query_sql for report ID 5 to use correct entity types for "dueno"
-- Should use id_tipo_entidad IN (4, 15, 17) instead of = 5
UPDATE reportes
SET query_sql = 'WITH cuentas_con_montos AS (
    SELECT 
      ...
      (
        ...
        AND er.id_tipo_entidad IN (4, 15, 17) 
        AND er.activo = true 
        LIMIT 1
      ) AS dueno,
      (
        ...
        AND er_dueno.id_tipo_entidad IN (4, 15, 17) 
    ...
    WHERE cc.activo = true 
      ...
      {{AND EXISTS (SELECT 1 FROM entidades_relacionadas er WHERE er.id_proyecto = pr.id AND er.id_tipo_entidad IN (4, 15, 17) AND er.activo = true AND er.id_persona = :id_dueno)}}
