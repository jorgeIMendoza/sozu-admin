...
-- Fix the function to use the correct table and column names
...
BEGIN
  ...
  LEFT JOIN estatus_cuenta_cobranza ecc ON cc.id_estatus_cuenta_cobranza = ecc.id
  ...
  SELECT 
    ...
    cc.id_estatus_cuenta_cobranza::integer AS id_estatus_cuenta,
    ecc.nombre::text AS estatus_cuenta_nombre,
  ...
  GROUP BY cc.id, cc.clabe_stp, cc.precio_final, cc.fecha_compra, cc.activo, 
           cc.porcentaje_comision_venta, cc.valor_uma, cc.id_oferta, cc.id_estatus_cuenta_cobranza,
           ecc.nombre, prop.id, prop.numero_propiedad, prop.piso, proy.id, proy.nombre,
