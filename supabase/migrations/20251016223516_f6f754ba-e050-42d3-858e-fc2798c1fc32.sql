-- 1. Actualizar función trigger para usar m2_interiores + m2_exteriores
...
-- 2. Crear función de incremento mensual
...
-- 3. Configurar cron job para ejecución mensual
-- Desactivar cualquier job existente con este nombre (ignorar error si no existe)
DO $$
BEGIN
    PERFORM cron.unschedule('incremento-mensual-precio-m2-proyectos');
EXCEPTION
    WHEN OTHERS THEN
        NULL; -- Ignorar el error si el job no existe
END $$;

-- Crear el nuevo job programado para ejecutarse el día 1 de cada mes a las 00:00 UTC
SELECT cron.schedule(
    'incremento-mensual-precio-m2-proyectos',
    '0 0 1 * *',
    $$SELECT incrementar_precio_m2_mensual()$$
);