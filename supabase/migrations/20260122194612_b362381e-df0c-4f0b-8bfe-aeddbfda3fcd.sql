-- Fix the RPC function - use 8025 UMAs for cash limit calculation
...
BEGIN
  ...
  SELECT 
    ...
    -- Metraje: m2_interiores + m2_exteriores
    COALESCE(prop.m2_interiores, 0) + COALESCE(prop.m2_exteriores, 0) as metraje,
    -- Precio lista
    prop.precio_lista,
    -- Pagado: sum of active payment applications
    COALESCE(
      (SELECT SUM(ap.monto) 
    ...
    -- Apartado pagado (primer acuerdo completado)
    COALESCE(
