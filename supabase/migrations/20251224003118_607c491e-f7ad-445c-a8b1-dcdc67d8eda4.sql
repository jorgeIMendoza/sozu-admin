-- Fix the date cast issue by removing ::date and using proper date format
UPDATE reportes 
SET query_sql = 'WITH pagos_propiedades AS (
  SELECT 
    ...
    p.numero AS numero_departamento,
    CONCAT(em.nombre, '' - '', m.nombre) AS edificio_modelo,
    pago.fecha_pago,
    pago.monto,
  ...
  JOIN proyectos pr ON em.id_proyecto = pr.id
  LEFT JOIN entidades_relacionadas er_dueno ON p.id_persona_dueno = er_dueno.id
  ...
  WHERE pago.activo = true
    ...
    {{AND date_trunc(''month'', pago.fecha_pago) = date_trunc(''month'', :mes_pago::timestamp)}}
),
pagos_productos AS (
  SELECT 
    ''producto'' AS tipo,
  ...
  JOIN propiedades p ON ps.id_propiedad = p.id
  JOIN edificios_modelos em ON p.id_edificio_modelo = em.id
  JOIN modelos m ON em.id_modelo = m.id
  JOIN proyectos pr ON em.id_proyecto = pr.id
  LEFT JOIN entidades_relacionadas er_dueno ON p.id_persona_dueno = er_dueno.id
