-- Fix report ID 5: id_comprador is in ofertas table, not cuentas_cobranza
UPDATE reportes
...
JOIN proyectos proy ON o.id_proyecto = proy.id
...
LEFT JOIN entidades_proyectos ep_dueno ON prop.id_entidad_relacionada_dueno = ep_dueno.id
LEFT JOIN personas dueno_persona ON ep_dueno.id_persona = dueno_persona.id
...
WHERE cc.activo = true
  ...
  {{AND ep_dueno.id_persona = :id_dueno}}
