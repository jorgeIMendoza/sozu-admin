...
BEGIN
  ...
  IF p_id_cuenta IS NOT NULL AND p_id_cuenta <> '' THEN
    BEGIN
      v_id_filter := CAST(REPLACE(LOWER(p_id_cuenta), 'cm-', '') AS INTEGER);
    EXCEPTION WHEN OTHERS THEN
      v_id_filter := NULL;
    END;
  END IF;

  -- Use a CTE-based approach for efficiency
  RETURN QUERY
  WITH
  -- Base: all active maintenance accounts with parent info
  base_accounts AS (
    SELECT
      cc.id AS cuenta_id,
      ...
      -- Parent property chain
      pp.id AS prop_id,
      pp.numero_propiedad::TEXT AS prop_numero,
      parent_cc.clave_catastral::TEXT AS prop_clave_catastral,
      pp.id_entidad_relacionada_dueno AS prop_entidad_dueno_id,
      pp.id_edificio_modelo AS prop_edificio_modelo_id,
      -- Project info via entidades_relacionadas -> proyectos
    ...
    -- Join propiedad -> edificio_modelo -> edificio + modelo
    ...
    WHERE cc.id_cuenta_cobranza_padre IS NOT NULL
      AND cc.activo = true
      -- Access control: project filter
      AND (p_proyecto_ids IS NULL OR proy.id = ANY(p_proyecto_ids))
      -- Access control: owner entity filter
  ...
  pagos_info AS (
    SELECT
      ...
      COALESCE(SUM(p.monto), 0) AS total_pagos_real
    FROM pagos p
    WHERE p.activo = true
      AND p.id_cuenta_cobranza IN (SELECT cuenta_id FROM base_accounts)
    GROUP BY p.id_cuenta_cobranza
  ),
  -- Compradores from the MAINTENANCE account itself (not parent)
  compradores_info AS (
    SELECT
    ...
    FROM compradores c
    LEFT JOIN personas per ON per.id = c.id_persona
  ...
  -- Próxima fecha de pago (max unpaid date)
  proxima_fecha AS (
    SELECT
      ap.id_cuenta_cobranza,
      MAX(ap.fecha_pago) AS fecha_maxima
    FROM acuerdos_pago ap
    WHERE ap.activo = true
  ...
  multas_info AS (
    ...
    FROM acuerdos_pago ap
    INNER JOIN multas m ON m.id_acuerdo_pago = ap.id
    WHERE ap.activo = true
      AND m.activo = true
      AND m.es_pagada = false
      AND ap.id_cuenta_cobranza IN (SELECT cuenta_id FROM base_accounts)
  ),
  -- Bodegas by parent property
  bodegas_info AS (
    SELECT
      ba.prop_id,
      COALESCE(jsonb_agg(
  ...
  -- Estacionamientos by parent property
  estacionamientos_info AS (
    SELECT
      ...
      COALESCE(jsonb_agg(
        jsonb_build_object(
          'nombre', e.nombre,
          'tipo', COALESCE(te.nombre, 'Sin tipo'),
          'm2', e.m2,
  ...
  -- Productos by parent property (from parent ofertas, excluding Bodega/Estacionamiento categories)
  productos_info AS (
    SELECT
      parent_of.id_propiedad AS prop_id,
      COALESCE(jsonb_agg(
        jsonb_build_object(
          'nombre', ps.nombre,
          'categoria', COALESCE(cp.nombre, 'Sin categoría'),
