-- Drop and recreate with correct table reference (compradores instead of ofertas_compradores)
...
BEGIN
  ...
  dueno_info AS (
    SELECT 
      cm.id_cuenta_cobranza_padre,
      per.nombre_legal AS dueno_nombre,
      cm.id_entidad_relacionada_dueno
    FROM cuentas_mantenimiento cm
    JOIN entidades_relacionadas er ON er.id = cm.id_entidad_relacionada_dueno
    JOIN personas per ON per.id = er.id_persona
  ...
  SELECT 
    ...
    COALESCE(di.dueno_nombre, prop_dueno.nombre_legal)::text AS dueno,
    COALESCE(di.id_entidad_relacionada_dueno, prop.id_entidad_relacionada_dueno)::integer,
    ...
    o.limite_efectivo::numeric AS cash_limit,
  ...
  LEFT JOIN dueno_info di ON di.id_cuenta_cobranza_padre = cc.id
