-- Drop existing function and recreate with correct column references
DROP FUNCTION IF EXISTS get_cuentas_cobranza_paginadas(
  integer, integer, text, text, text, text, text, text, text, integer[], text[], boolean, integer[], integer[]
);
...
BEGIN
  ...
  -- Get total count first (using o.id_propiedad and o.id_producto from ofertas table)
  SELECT COUNT(*)
  INTO v_total
  ...
  LEFT JOIN proyectos proy ON COALESCE(prop.id_proyecto, ps.id_proyecto) = proy.id
  ...
  LEFT JOIN entidades_relacionadas_proyectos erp ON COALESCE(prop.id_proyecto, ps.id_proyecto) = erp.id_proyecto 
  ...
  WHERE cc.activo = p_activo
    AND (p_proyecto_ids IS NULL OR COALESCE(prop.id_proyecto, ps.id_proyecto) = ANY(p_proyecto_ids))
    ...
    AND (p_tipos IS NULL OR 
         (('Propiedad' = ANY(p_tipos) AND o.id_propiedad IS NOT NULL) OR
          ('Producto' = ANY(p_tipos) AND o.id_producto IS NOT NULL AND o.id_propiedad IS NULL)))
  ...
  SELECT 
    ...
    COALESCE(prop.id_proyecto, ps.id_proyecto) as id_proyecto,
    ...
    erp.id_entidad_relacionada::integer as id_entidad_relacionada_dueno,
    (
