...
-- Drop and recreate the function with corrected tipo classification
...
BEGIN
  ...
  WHERE cc.activo = p_activo
    ...
    AND (p_tipos IS NULL OR (
      CASE
        ...
        WHEN ps.id IS NOT NULL AND ps.id_categoria IS NOT NULL THEN 'Producto'
        ELSE 'Servicio'
      END
    ) = ANY(p_tipos))
  ...
  SELECT
    ...
    CASE 
      WHEN cc.valor_uma IS NOT NULL AND cc.valor_uma > 0 
      THEN 8025 * cc.valor_uma 
      ELSE NULL 
    END AS cash_limit,
    ...
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object('fecha_pago', p.fecha_pago, 'monto', p.monto)) 
       FROM pagos p WHERE p.id_cuenta_cobranza = cc.id AND p.activo = true AND p.id_metodos_pago = 1),
      '[]'::jsonb
    ) AS cash_payments,
