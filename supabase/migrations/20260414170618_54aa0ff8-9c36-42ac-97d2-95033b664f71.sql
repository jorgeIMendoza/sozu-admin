...
CREATE OR REPLACE FUNCTION public.get_bandeja_operativa(
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        COALESCE(SUM(CASE WHEN ap.pago_completado = false AND ap.fecha_pago < v_hoy THEN ap.monto END), 0) AS monto_vencido,
        COALESCE(SUM(CASE WHEN ap.pago_completado = false THEN ap.monto END), 0) AS saldo_pendiente,
