...
-- Recreate with corrected joins (propiedades -> edificios_modelos -> id_proyecto)
...
BEGIN
  ...
  WHERE cc.activo = p_activo
    ...
    AND (p_proyecto_ids IS NULL OR COALESCE(em.id_proyecto, ps.id_proyecto) = ANY(p_proyecto_ids))
  ...
  SELECT 
    ...
    COALESCE(em.id_proyecto, ps.id_proyecto) AS id_proyecto,
  ...
  LEFT JOIN productos_servicios ps ON o.id_producto = ps.id
  LEFT JOIN proyectos proy ON COALESCE(em.id_proyecto, ps.id_proyecto) = proy.id
