-- Corregir el query del reporte "Completamente liquidados" (ID 7)
-- El problema es que cuando una oferta tiene AMBOS id_propiedad e id_producto,
-- significa que se vendió un PRODUCTO asociado a esa propiedad, no la propiedad en sí.
-- Debemos priorizar id_producto sobre id_propiedad para determinar el tipo correcto.
...
SELECT
  ...
  CASE 
    WHEN o.id_producto IS NOT NULL THEN p.numero_propiedad
...
LEFT JOIN proyectos pr ON (
  CASE 
    WHEN o.id_producto IS NOT NULL THEN ps.id_proyecto
...
WHERE cc.activo = true
  ...
  AND (
    (o.id_propiedad IS NOT NULL AND o.id_producto IS NULL AND p.id_estatus_disponibilidad = 5)
    OR (o.id_producto IS NOT NULL)
  )
  AND cc.precio_final <= COALESCE(ppc.total_pagado, 0)
  {{AND pr.id = :id_proyecto}}
  {{AND CASE WHEN o.id_producto IS NOT NULL THEN ''Producto'' WHEN o.id_propiedad IS NOT NULL THEN ''Propiedad'' ELSE ''Desconocido'' END = :tipo}}
